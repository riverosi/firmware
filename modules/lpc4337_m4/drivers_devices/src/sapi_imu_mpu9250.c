...
#include "chip.h"           /* <= I2C chip header */
#include "stopwatch.h"
...
static int8_t mpu9250WriteRegister( uint8_t subAddress, uint8_t data )
{
	...
	I2CM_XFER_T i2cData;
	i2cData.slaveAddr = control.address;
	i2cData.options   = 0;
	i2cData.status    = 0;
...
static int8_t mpu9250ReadRegisters( uint8_t subAddress, uint8_t count )
{
	...
	i2cData.options   = 0;
	i2cData.status    = 0;
	i2cData.txBuff    = &subAddress;
	i2cData.txSz      = 1;
	i2cData.rxBuff    = control._buffer;
	i2cData.rxSz      = count;
...
static int8_t mpu9250WhoAmI( void )
{
	...
	return (int8_t)control._buffer[0];
}
...
static int8_t mpu9250WhoAmIAK8963( void )
{
	...
	// return the register value
	return (uint8_t)control._buffer[0];
}
...
static int8_t mpu9250ReadAK8963Registers( uint8_t subAddress, uint8_t count )
{
	...
	StopWatch_DelayMs(1); // takes some time for these registers to fill
...
int mpu9250Init( MPU9250_address_t address )
{
   ...
   // setting the gyro scale to 2000DPS
	control._gyroScale = 250.0f / 32767.5f * MPU9250_D2R;
	control._gyroRange = MPU9250_GYRO_RANGE_250DPS;
